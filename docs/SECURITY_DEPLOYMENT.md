...
1. Generate new independent values for `SECRET_KEY`, `JWT_SECRET_KEY`, database password, and `WAHA_WEBHOOK_SECRET`.
...
4. Configure WAHA to send `X-Webhook-Secret` with the same `WAHA_WEBHOOK_SECRET` value.
